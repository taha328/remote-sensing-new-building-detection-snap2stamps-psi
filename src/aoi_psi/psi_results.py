...
def load_ps_points(points_csv: Path, *, target_crs: str | None = None) -> gpd.GeoDataFrame:
    ...
    if {"lon", "lat"}.issubset(frame.columns):
        ...
        return points if target_crs is None else points.to_crs(target_crs)

    if {"x", "y"}.issubset(frame.columns):
        if target_crs is None:
            raise ValueError("target_crs is required when PSI export only contains projected x/y coordinates.")
...
def load_points_only(points_csv: Path, *, target_crs: str | None = None) -> PsiArtifacts:
