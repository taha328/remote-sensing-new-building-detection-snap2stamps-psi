...
from casablanca_builtup.acquisition.stac import build_period_manifests, sign_manifest_items
from casablanca_builtup.config import PipelineConfig, load_config
from casablanca_builtup.fusion import apply_soft_refinement
from casablanca_builtup.grid import build_grid, load_aoi_frame
from casablanca_builtup.io import write_json, write_vector, write_yaml
from casablanca_builtup.logging_utils import configure_logging
from casablanca_builtup.postprocess.vectorize import (
    build_cumulative_first_change,
    build_density_zone_mask,
    polygonize_mask,
)
from casablanca_builtup.qa import (
    base_run_report,
    binary_area_ha,
    binary_pixel_count,
    decision_histogram,
...
from casablanca_builtup.resume import (
    export_vector_path,
...
from casablanca_builtup.runtime import PipelineInterruptedError, interruption_guard, log_timing
from casablanca_builtup.s1.composite import build_s1_composite
from casablanca_builtup.s1.detection import detect_s1_change
from casablanca_builtup.s2.composite import build_s2_composite
from casablanca_builtup.s2.refinement import build_s2_support, build_unavailable_s2_support_like
        ...
        with interruption_guard():
            ...
            for period in config.periods:
                ...
                if s1_before is None:
                    ...
                    save_s1_composite(s1_before, context, period.id, "before", config.grid.crs, config.export)
                ...
                if s1_after is None:
                    ...
                    save_s1_composite(s1_after, context, period.id, "after", config.grid.crs, config.export)
                ...
                if s1_detection is None:
                    ...
                    save_s1_detection(s1_detection, context, period.id, config.grid.crs, config.export)
                ...
                if refinement_artifacts is None:
                    ...
                    save_refinement(s2_support, fusion, context, period.id, config.grid.crs, config.export)
            ...
            if polygons is None:
                polygons = polygonize_mask(zone_mask, config.grid.crs, config.polygonization)
