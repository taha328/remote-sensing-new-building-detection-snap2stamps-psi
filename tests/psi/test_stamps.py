...
def test_run_stack_reuses_existing_stamps_outputs_and_cleans_snap_export(tmp_path, monkeypatch) -> None:
    ...
    assert not snap_export_dir.exists()
    assert any(record.category == "snap_export" for record in outputs.cleanup_records)


def test_run_stack_cleans_snap_export_after_valid_stamps_outputs(tmp_path, monkeypatch) -> None:
    root = Path(__file__).resolve().parents[2]
    config = load_config(root / "configs" / "psi_casablanca_slc_minimal.yaml")
    runner = StaMPSRunner(config)
    monkeypatch.setattr(runner, "validate_environment", lambda: None)
    monkeypatch.setattr(runner, "_run_mt_prep_snap", lambda *args, **kwargs: None)
...
def test_environment_includes_stamps_bin_and_interpreter_dir(monkeypatch) -> None:
    root = Path(__file__).resolve().parents[2]
    config = load_config(root / "configs" / "psi_casablanca_slc_minimal.yaml")
    runner = StaMPSRunner(config)
    ...
    assert path_parts[1] == "/Applications/MATLAB_R2025b.app/bin"
...
def test_describe_execution_plan_is_patch_aware() -> None:
    root = Path(__file__).resolve().parents[2]
    config = load_config(root / "configs" / "psi_casablanca_slc_minimal.yaml")
    runner = StaMPSRunner(config)
...
def test_run_stack_uses_parallel_patch_phase_then_serial_merge(tmp_path, monkeypatch) -> None:
    ...
    assert "stamps(5,8,[],0,[],2);" in serial_scripts[0][0]
    assert "stamps(1,8);" not in serial_scripts[0][0]
...
def test_run_stack_executes_patch_batches_serially_when_parallel_limit_is_one(tmp_path, monkeypatch) -> None:
    ...
    assert "stamps(1,5,[],0,'patch_list_split_1',1);" in serial_scripts[0][0]
    assert "stamps(1,5,[],0,'patch_list_split_4',1);" in serial_scripts[3][0]
    assert "stamps(5,8,[],0,[],2);" in serial_scripts[4][0]
