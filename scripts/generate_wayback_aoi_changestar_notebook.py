...
NOTEBOOK_PATH = Path(__file__).resolve().parents[1] / "notebooks" / "aoi_new_building_wayback_changestar_kaggle.ipynb"
...
def build_notebook() -> dict:
    cells = [
        markdown_cell(
            """
            # AOI New-Building Detection with Wayback + Changen2-Pretrained ChangeStar

            This notebook uses the **official public Changen2-pretrained ChangeStar inference path** to detect **new buildings** between two ArcGIS Wayback releases over AOI.
        ...
        code_cell(
            """
            OUTPUT_DIR = Path("/kaggle/working/aoi_wayback_changestar")
